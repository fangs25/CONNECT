#!/usr/bin/env python3
"""
Mapping Metrics Calculator - RNA+ADT Version
专门用于RNA+ADT模态的mapping指标计算
"""
...
class RNADTMappingMetricsCalculator:
    """RNA+ADT模态的Mapping指标计算器"""

    def __init__(self):
        self.output_dir = "mapping_results"
        self.modality = "RNA+ADT"
        self.ensure_output_directories()

        print(f"RNA+ADT Mapping Metrics Calculator 初始化完成")
    ...
    def load_rna_adt_mapping_data(self, dataset_name: str) -> Dict[str, Any]:
        """
        加载RNA+ADT模态的mapping数据
        """
        print(f"加载 {dataset_name} 的RNA+ADT mapping数据...")
        ...
        try:
            ...
            adt_data = sc.read_h5ad(f'/DATA2/zhangjingxiao/fangs/models_2025/Model_results/1205midas_map/{dataset_name}/adt_test.h5ad')
            ...
            for key in rna_data.uns.keys():
                if key.endswith('_rna_mapped'):
                    ...
                    if f'{method}_adt_mapped' in adt_data.uns:
    ...
    def calculate_rna_adt_method_metrics(self, rna_data, adt_data, method: str) -> Dict[str, Any]:
        """
        计算RNA+ADT模态单个方法的mapping指标
        """
        print(f"\n计算RNA+ADT方法: {method}")
        ...
        try:
            ...
            print(f'  ADT嵌入形状: {adt_emb.shape}')
            print(f'  RNA映射形状: {rna_mapped.shape}')
            print(f'  ADT映射形状: {adt_mapped.shape}')

            # 使用全部数据，不进行抽样
            print(f'  使用全部 {rna_emb.shape[0]} 个细胞计算RNA+ADT mapping指标')

            # 计算FOSCTTM
            print(f'  📊 计算FOSCTTM (RNA+ADT)...')
            ...
            print(f'  ADT FOSCTTM: 均值={np.mean(foscttm_adt_full):.4f}, 标准差={np.std(foscttm_adt_full):.4f}')
            print(f'  RNA FOSCTTM: 均值={np.mean(foscttm_rna_full):.4f}, 标准差={np.std(foscttm_rna_full):.4f}')

            # 计算Top-K Accuracy
            print(f'  📊 计算Top-K Accuracy (RNA+ADT)...')
            ...
            return {
                'foscttm_adt_full': foscttm_adt_full.tolist(),
            ...
            print(f"  ❌ 计算RNA+ADT方法 {method} 失败: {e}")
            return None

    def calculate_rna_adt_dataset_metrics(self, dataset_name: str) -> Dict[str, Any]:
        """
        计算单个数据集的RNA+ADT模态mapping指标
        """
        print(f"\n{'='*80}")
        print(f"计算RNA+ADT数据集: {dataset_name}")
        print(f"{'='*80}")

        start_time = time.time()

        # 加载数据
        data_info = self.load_rna_adt_mapping_data(dataset_name)
        ...
        for method in available_methods:
            method_results = self.calculate_rna_adt_method_metrics(rna_data, adt_data, method)
            if method_results:
        ...
        # 保存结果
        self.save_rna_adt_foscttm_csv(dataset_name, results)
        self.save_rna_adt_topk_json(dataset_name, results)

        end_time = time.time()
        results['processing_time'] = end_time - start_time

        print(f"\n✅ RNA+ADT数据集 {dataset_name} 计算完成，耗时: {results['processing_time']:.2f} 秒")

        return results

    def save_rna_adt_foscttm_csv(self, dataset_name: str, results: Dict[str, Any]):
        """
        保存RNA+ADT模态FOSCTTM细胞级别数据到CSV文件
        """
        foscttm_data_adt_to_rna = []
        foscttm_data_rna_to_adt = []
        ...
        for cell_id in range(n_cells):
            ...
            for method, method_data in results['methods'].items():
                if method_data.get('status') != 'failed' and 'foscttm_adt_full' in method_data:
                    # ADT方向的FOSCTTM值
                    if cell_id < len(method_data['foscttm_adt_full']):
                        row[method] = method_data['foscttm_adt_full'][cell_id]
                    else:
                        row[method] = None
                else:
                    row[method] = None

            foscttm_data_rna_to_adt.append(row)

        # 创建DataFrame
        df = pd.DataFrame(foscttm_data_rna_to_adt)
        # 保存到文件
        output_file = os.path.join(self.output_dir, "foscttm", f"{dataset_name}_rna_to_adt_foscttm.csv")
        ...
        for cell_id in range(n_cells):
            ...
            foscttm_data_adt_to_rna.append(row)

        df = pd.DataFrame(foscttm_data_adt_to_rna)
        output_file = os.path.join(self.output_dir, "foscttm", f"{dataset_name}_adt_to_rna_foscttm.csv")
        df.to_csv(output_file, index=False)

        print(f"  ✓ RNA+ADT FOSCTTM数据已保存: {output_file}")
        print(f"    形状: {df.shape} (细胞 x 方法)")

    def save_rna_adt_topk_json(self, dataset_name: str, results: Dict[str, Any]):
        """
        保存RNA+ADT模态Top-K Accuracy数据到JSON文件
        """
        topk_data = {}
        ...
        # 保存到文件
        output_file = os.path.join(self.output_dir, "topk_accuracy", f"{dataset_name}_rna_adt_topk_accuracy.json")

        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(topk_data, f, indent=2, ensure_ascii=False)

        print(f"  ✓ RNA+ADT Top-K数据已保存: {output_file}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Calculate RNA+ADT mapping metrics from scratch',
    ...
    parser.add_argument(
        ...
        type=str,
        action='append',
        help='RNA+ADT dataset name(s) to calculate (can be specified multiple times)'
    )
    ...
    try:
        calculator = RNADTMappingMetricsCalculator()

        for dataset_name in args.dataset:
            calculator.calculate_rna_adt_dataset_metrics(dataset_name)

        print(f"\n📁 输出目录: {calculator.output_dir}")
        print(f"   - RNA+ADT FOSCTTM数据: {os.path.join(calculator.output_dir, 'foscttm/')}")
        print(f"   - RNA+ADT Top-K数据: {os.path.join(calculator.output_dir, 'topk_accuracy/')}")

        print(f"\n✅ RNA+ADT Mapping指标计算完成，无图片生成！")
